...
class Dataset_Load(Dataset):
    ...
    def __getitem__(self, index):
        hazy_image_name = str(index+1 ) + opt.img_extension
        hazy_im = cv2.imread(os.path.join(self.hazy_dir, "img"+hazy_image_name))

        hazy_im = hazy_im[:, :, ::-1] ## BGR to RGB   R
        hazy_im = np.float32(hazy_im) / 255.0


        clean_image_name = str(index+1 ) + opt.img_extension
        clean_im = cv2.imread(os.path.join(self.clean_dir, "img"+clean_image_name))
